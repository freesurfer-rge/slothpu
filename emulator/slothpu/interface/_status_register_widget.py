...
class StatusRegisterWidget(urwid.WidgetWrap):
    def __init__(self, status_register: StatusRegister):
        ...
        sr_box = urwid.LineBox(status_pile, title="Status Register", title_align=urwid.LEFT)
        self.update()
    ...
    def update(self):
        ...
        self._salu_text.set_text(f"SALU: {self._sr.value[StatusRegister.salu_bit] == 1}")
        self._dalu_text.set_text(f"DALU: {self._sr.value[StatusRegister.dalu_bit] == 1}")